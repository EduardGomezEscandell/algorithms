...
namespace internal
{
    ...
    big_uint<D> most_significant_n(big_uint<D> input, std::size_t n) {
        return big_uint<D>(
            std::next(input.digits_.begin(), n),
            input.digits_.end()
        );
    }
    ...
    std::tuple<D, D> prod_digits(D lhs, D rhs) {
        if(lhs == 0 || rhs == 0) return std::make_tuple(0, 0);
...
class big_uint {
    ...
    big_uint(ForwardIterator begin, ForwardIterator end) 
     : digits_(begin, end)
    { }
    ...
    bool operator<(big_uint const& other) const { return compare(other) == comparison::LT; }
    bool operator>(big_uint const& other) const { return compare(other) == comparison::GT; }
    bool operator==(big_uint const& other) const { return compare(other) == comparison::EQ; }
    bool operator!=(big_uint const& other) const { return !(*this == other); }
    bool operator<=(big_uint const& other) const { return !(*this > other); }
    bool operator>=(big_uint const& other) const { return !(*this < other); }
...
public:
    ...
    friend
    big_uint operator+(const big_uint& lhs, const big_uint& rhs)
    {
        ...
        result.digits_ =
        std::inner_product(lhs.digits_.cbegin(),
            lhs.digits_.cend(),
            rhs.digits_.cbegin(),
            result.digits_,
            reduction,
            internal::sum_digits<D>
        );
    ...
    big_uint operator-(const big_uint& lhs, big_uint rhs)
    {
        // Two's complement
    ...
    big_uint operator*(const big_uint& lhs, const big_uint& rhs)
    {
        ...
        if(lhs.digits_.size() == 1 && rhs.digits_.size() == 1) {
            ...
            if(d1 != 0) result.digits_.push_back(d1);
            return result;
        }
    ...
    friend
    std::ostream& operator<<(std::ostream& os, const big_uint& longint)
    {    
